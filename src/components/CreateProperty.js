...
const CreateProperty = () => {
  ...
  const initialState =
    singleData && (singleData.is_area == "1") !== false ? true : false;
  ...
  const [checkedSwitch, setChecked] = useState(initialState);
  const [location, setAddress] = useState("");
  // console.log("in", initialState);
  // console.log("chk", singleData && singleData.is_area == "1" ? true : false);
  ...
  // const initialState = singleData && singleData.is_area == 1 ? true : false;
  // const [checkedSwitch, setChecked] = useState(initialState);
  // console.log("is_area_chk", initialState);

  // const initialState = singleData && singleData.is_area == 0 ? false : true;
  ...
  useEffect(() => {
    ...
    localStorage.setItem("ready_to_sell", "");
    localStorage.setItem("garden_type", "");
    localStorage.setItem("parking", "");
    localStorage.setItem("style", "");
    localStorage.setItem("condition", "");
    localStorage.setItem("windows", "");
    localStorage.setItem("period_features", "");
    localStorage.setItem("potential_expand", "");
    localStorage.setItem("heating", "");
    localStorage.setItem("switchValue", "");
  }, [dispatch]);

  var dataobj = {};
  for (let key in singleData) {
    dataobj[key] = singleData[key];
    if (key == "building" || key == "external") {
      Object.keys(dataobj[key]).forEach(function (subkey) {
        if (subkey != "id" && subkey != "pr_id") {
          // localStorage.setItem(key, dataobj['building'][subkey]);
          dataobj[subkey] = dataobj[key][subkey];
        }
      });
    }
    if (dataobj["is_area"] && localStorage.getItem("switchValue") === "") {
      var switchValue = dataobj["is_area"] == 0 ? false : true;
  ...
  const handleChangeCheckbox = (e) => {
    if (localStorage.getItem(e.target.name) === "") {
      ...
      localStorage.setItem(e.target.name, tempType);
    } else {
      propertyType = localStorage.getItem(e.target.name);
    }
    ...
    localStorage.setItem(e.target.name, propertyType);
  ...
  const onUpdate = (e) => {
    ...
    dataobj.latitude = localStorage.getItem("lat");
    dataobj.longitude = localStorage.getItem("lng");
    dispatch(updateProperty(dataobj, history));
  };
                          ...
                          <div className="form-row  content-responsive">
                            ...
                            <div className="col-sm-4 ">
                              <input
                                ...
                                defaultChecked={
                                  singleData.ready_to_sell &&
                                  singleData.ready_to_sell.includes(
                                    "Mingle"
                                  ) !== false
                                    ? "checked"
                                    : ""
                    ...
                    <div className="form-group col-sm-6">
                      <div>
                        ...
                        <PlacesAutocomplete
                          ...
                          // getDefaultValue={singleData.location}
                          // defaultValue={() => setAddress(singleData.location)}
                        >
                          {({
                            getInputProps,
                            suggestions,
                            ...
                            <div>
                              <input
                                {...getInputProps({
                                  placeholder: singleData.location,

                                  className: "property-places-input",
                                })}
                                required
                              />
